        ...
        <form @submit.prevent="handleSave" class="flex flex-col gap-5">
          ...
          <div v-show="activeTab === 'info'" class="space-y-5">
            ...
            <div class="pt-4 border-t border-border-main">
              <h4 class="text-xs font-bold text-text-main uppercase tracking-wider mb-3">💳 Tài Khoản Nhận Thanh Toán (VietQR)</h4>
              <div class="mb-4">
                <FormSelect
            ...
            <div class="pt-4 border-t border-border-main">
              <div class="flex justify-between items-center mb-3">
                <h4 class="text-xs font-bold text-text-main uppercase tracking-wider m-0">🔌 Dịch Vụ & Phụ Phí Định Kỳ</h4>
                <FormButton type="button" @click="addExtraFeeRow" variant="secondary" size="sm" class="!px-2.5 !py-1 text-xs">
                  + Thêm dịch vụ
                </FormButton>
              </div>
          ...
          <div v-show="activeTab === 'rules'" class="space-y-5">
            ...
            <div class="mb-4">
              ...
              <textarea
                ...
                class="w-full px-3 py-2.5 text-xs border border-border-main bg-card text-text-main rounded-lg outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 leading-relaxed font-mono"
              ></textarea>
            </div>
          </div>
        </form>
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="bg-card border border-border-main rounded-2xl p-6 shadow-xs">
          ...
          <div class="flex flex-col gap-4 text-xs">
            ...
            <div class="flex justify-between items-center py-1 border-b border-border-main/40">
              ...
              <span class="font-bold text-text-main text-primary">{{ formatMoney(house.defaultElectricityRate) }} đ/kWh</span>
            </div>
            <div class="flex justify-between items-center py-1 border-b border-border-main/40">
              <span class="text-text-sub">Đơn giá nước:</span>
              <span class="font-bold text-text-main text-primary">{{ formatMoney(house.defaultWaterRate) }} đ ({{ house.waterBillingType === 'BY_INDEX' ? 'Đồng hồ' : 'Đầu người' }})</span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-[rgba(0,102,204,0.04)] border border-primary/20 rounded-2xl p-5">
          ...
          <p class="text-[11px] leading-relaxed text-text-sub">
            Việc thiết lập đầy đủ tài khoản ngân hàng và các quy định nội quy rõ ràng sẽ giúp tối ưu hóa trải nghiệm tự động hóa dịch vụ cho khách thuê của bạn. Khách thuê sẽ nhìn thấy mã QR VietQR khi nhận hóa đơn và xem được bảng nội quy dãy trọ này tại trang riêng của họ.
          </p>
        </div>
      </div>
    </div>

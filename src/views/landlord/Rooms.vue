    ...
    <PageHeader title="Quản Lý Phòng Trọ" subtitle="Thêm mới, sửa thông tin phòng trọ và theo dõi trạng thái phòng"
      ...
      <template #filter-addons>
        <div style="display: flex; align-items: center; gap: 0.5rem; margin-left: auto;">
          <span style="font-weight: 500; font-size: 0.85rem; color: var(--text-secondary); white-space: nowrap;">Dãy
            trọ:</span>
          <select class="form-select" style="width: 200px; padding: 0.4rem 0.75rem; font-size: 0.85rem;"
      ...
      <div v-else>
            ...
            <tbody>
              <tr v-for="room in filteredRooms" :key="room.id">
                ...
                <td style="text-align: right; display: flex; gap: 0.25rem; justify-content: flex-end;">
                  <button @click="editRoom(room)" class="btn btn-outline"
                    style="padding: 0.4rem 0.8rem; font-size: 0.85rem;">
                    Sửa
                  </button>
                  <button @click="deleteRoom(room.id)" class="btn btn-danger"
                    style="padding: 0.4rem 0.8rem; font-size: 0.85rem;" :disabled="room.status === 'OCCUPIED'">
                    Xóa
                  </button>
                </td>
              </tr>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 1.5rem;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            <button class="btn btn-outline" style="padding: 0.5rem 1rem;" :disabled="page === 0"
              @click="changePage(page - 1)">
              Trước
            </button>
            <button class="btn btn-outline" style="padding: 0.5rem 1rem;" :disabled="page >= totalPages - 1"
              @click="changePage(page + 1)">
              Sau
            </button>
          </div>

      ...
      <div
        ...
        <form @submit.prevent="saveInvoice">
          ...
          <div v-if="selectedContract"
            ...
            <label v-if="selectedContract.room.boardingHouse.billingTiming === 'PREPAID'"
              class="flex items-start gap-2.5 cursor-pointer text-xs font-medium text-text-main">
              <input type="checkbox" v-model="form.excludeRoomPrice"
                class="mt-0.5 rounded border-border-main text-primary focus:ring-primary cursor-pointer" />
              <div class="flex flex-col">
                <span>Không thu tiền phòng (Chỉ tính điện nước)</span>
                <span class="text-[10px] text-text-sub font-normal">Sử dụng cho kỳ hóa đơn cuối cùng trước khi trả phòng
                  (do tiền phòng đã đóng trước đó).</span>
              </div>
            </label>

            <label class="flex items-start gap-2.5 cursor-pointer text-xs font-medium text-text-main">
              <input type="checkbox" v-model="form.excludeExtraFees"
                class="mt-0.5 rounded border-border-main text-primary focus:ring-primary cursor-pointer" />
              <div class="flex flex-col">
                <span>Không thu các phụ phí dịch vụ</span>
                <span class="text-[10px] text-text-sub font-normal">Sử dụng để bỏ qua các phí dịch vụ định kỳ (Wifi, vệ
                  sinh...) khi thanh lý hợp đồng.</span>
              </div>
            </label>
          </div>
    ...
    <div v-if="showDetailModal && invoiceDetails"
      ...
      <div
        ...
        <div id="receipt-print-area"
          class="font-sans text-black bg-white p-3 sm:p-4 border border-slate-100 rounded-xl">
          <div class="flex flex-col items-center justify-between border-b-2 border-slate-200 pb-5 mb-6">
            <div class="flex flex-col">
              <h2 class="font-bold text-lg text-center sm:text-lg text-slate-800 tracking-tight">HÓA ĐƠN TIỀN PHÒNG &
                DỊCH VỤ</h2>
              <div class="flex justify-between gap-4">
                <p class="text-[10px] sm:text-xs text-slate-500">
                  Kỳ hóa đơn: {{ formatDate(invoiceDetails.billingPeriodStart) }} -
                  {{ formatDate(invoiceDetails.billingPeriodEnd) }}
                </p>

                <p class="text-[10px] sm:text-xs text-slate-500">
                  Mã hóa đơn: #{{ invoiceDetails.id.substring(0, 8).toUpperCase() }}
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 text-[11px] sm:text-xs pb-5 mb-5 border-b border-slate-100 text-slate-700">
            <div class="space-y-1.5">
              <div><span class="text-slate-400 font-medium">Khách thuê:</span> <strong class="text-slate-800">{{
                invoiceDetails.contract.tenant.fullName }}</strong></div>
              <div><span class="text-slate-400 font-medium">Số điện thoại:</span> <span
                  class="text-slate-600 font-medium">{{ invoiceDetails.contract.tenant.phone || 'Chưa cập nhật'
                  }}</span></div>
              <div><span class="text-slate-400 font-medium">Địa chỉ trọ:</span> <span
                  class="text-slate-600 font-medium">{{ invoiceDetails.contract.room.boardingHouse.address ||
                    'Chưa cập nhật' }}</span></div>
            </div>
            <div class="space-y-1.5 text-right">
              <div><span class="text-slate-400 font-medium">Dãy trọ:</span> <strong class="text-slate-800">{{
                invoiceDetails.contract.room.boardingHouse.name }}</strong></div>
              <div><span class="text-slate-400 font-medium">Phòng số:</span> <strong class="text-primary">Phòng {{
                invoiceDetails.contract.room.roomNumber }}</strong></div>
              <div><span class="text-slate-400 font-medium">Ngày lập HĐ:</span> <span
                  class="text-slate-600 font-medium">{{ formatDate(invoiceDetails.invoiceDate) }}</span></div>
            </div>
          </div>
          ...
          <div class="overflow-x-auto border border-slate-100 rounded-xl mb-6">
            <table
              class="print-only-table min-w-[460px] md:min-w-0 w-full text-[11px] sm:text-xs border-collapse table-fixed">
              <thead>
                <tr class="border-b border-slate-200 bg-slate-50">
                  <th class="text-left py-2 px-2 w-[30%] font-bold text-slate-600">Khoản chi phí</th>
                  <th class="text-right py-2 px-2 w-[30%] font-bold text-slate-600">Đơn giá</th>
                  <th class="text-right py-2 px-2 w-[10%] font-bold text-slate-600 text-center">SL</th>
                  <th class="text-right py-2 px-2 w-[30%] font-bold text-slate-600">Thành tiền</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-dashed divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/30">
                  ...
                  <td class="text-right py-2.5 px-2 text-slate-600">{{
                    formatMoney(invoiceDetails.contract.contractedRoomPrice) }} đ</td>
                  <td class="text-right py-2.5 px-2 text-slate-600 text-center">1</td>
                  <td class="text-right py-2.5 px-2 font-bold text-slate-800">{{ formatMoney(invoiceDetails.roomPrice)
                  }} đ</td>
                </tr>
                <!-- Tiền điện -->
                <tr class="hover:bg-slate-50/30">
                  <td class="py-2.5 px-2 text-slate-800 font-medium break-words">
                    Tiền điện
                    <div class="text-[9px] text-slate-400 font-normal mt-0.5">Chỉ số: {{
                      invoiceDetails.newElectricityIndex }} - {{ invoiceDetails.oldElectricityIndex }} ({{
                        invoiceDetails.newElectricityIndex - invoiceDetails.oldElectricityIndex }} kWh)</div>
                  </td>
                  <td class="text-right py-2.5 px-2 text-slate-600">{{ formatMoney(invoiceDetails.electricityRate) }} đ
                  </td>
                  <td class="text-right py-2.5 px-2 text-slate-600 text-center">{{ invoiceDetails.newElectricityIndex -
                ...
                <tr class="hover:bg-slate-50/30">
                  <td class="py-2.5 px-2 text-slate-800 font-medium break-words">
                    Tiền nước
                    <div v-if="invoiceDetails.contract.room.boardingHouse.waterBillingType === 'BY_INDEX'"
                      class="text-[9px] text-slate-400 font-normal mt-0.5">Chỉ số: {{ invoiceDetails.newWaterIndex }} -
                      {{ invoiceDetails.oldWaterIndex }} ({{ invoiceDetails.newWaterIndex - invoiceDetails.oldWaterIndex
                      }} m³)</div>
                    <div v-else-if="invoiceDetails.contract.room.boardingHouse.waterBillingType === 'FIXED_PER_PERSON'"
                      class="text-[9px] text-slate-400 font-normal mt-0.5">Cố định theo đầu người</div>
                  ...
                  <td class="text-right py-2.5 px-2 text-slate-600 text-center">
                    ...
                    <span
                      v-else-if="invoiceDetails.contract.room.boardingHouse.waterBillingType === 'FIXED_PER_PERSON'">
          ...
          <div class="border-t-2 border-slate-200 pt-4 text-[11px] sm:text-xs flex justify-end">
            <div class="w-full sm:w-[50%] space-y-2">
              ...
              <div
                class="flex justify-between font-extrabold text-xs sm:text-sm border-t border-slate-100 pt-2 text-slate-900">
                <span>Còn lại cần thu:</span>
                <span class="text-rose-600">{{ formatMoney(invoiceDetails.totalAmount - invoiceDetails.paidAmount) }}
                  đ</span>
              </div>
            </div>
          </div>

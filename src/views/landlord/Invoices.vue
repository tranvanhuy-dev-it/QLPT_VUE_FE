      ...
      <div
        ...
        <div class="flex flex-wrap gap-2 justify-end mt-6">
          ...
          <button v-if="invoiceDetails.status !== 'PAID'" @click="payInvoiceFromDetails"
            class="px-3 py-1.5 text-xs font-bold rounded-lg bg-secondary text-white hover:bg-secondary-hover cursor-pointer transition-all">
            Thu trước
          </button>
          <button v-if="invoiceDetails.status !== 'PAID'" @click="quickPayInvoice"
            class="px-3 py-1.5 text-xs font-bold rounded-lg bg-primary text-white hover:bg-primary-hover cursor-pointer transition-all">
            Thu đủ
          </button>
        </div>
      </div>

  ...
  <div class="app-container">
    ...
    <main class="main-content">
      <header style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <div>
          <h1 style="font-weight: 700; font-size: 1.75rem;">Hóa Đơn & Thanh Toán</h1>
          <p style="color: var(--text-secondary);">Ghi chỉ số điện nước, lập hóa đơn phòng hàng tháng và theo dõi nợ</p>
        </div>
        <button @click="openCreateModal" class="btn btn-primary" :disabled="activeContracts.length === 0">
          + Lập Hóa Đơn Tháng
        </button>
      </header>
              ...
              <tbody>
                ...
                <tr v-if="invoices.length === 0">
                  <td colspan="8" style="text-align: center; color: var(--text-secondary); padding: 2rem;">
                    Chưa lập hóa đơn nào. Nhấp "Lập Hóa Đơn Tháng" để bắt đầu tính tiền phòng!
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div v-if="showCreateModal" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; justify-content: center; align-items: center; z-index: 1000;">
        <div class="card" style="width: 100%; max-width: 550px; padding: 2rem; margin: 1rem; position: relative;">
          ...
          <form @submit.prevent="saveInvoice">
            ...
            <div v-if="selectedContract" style="background: rgba(79, 70, 229, 0.05); padding: 0.75rem 1rem; border-radius: 8px; font-size: 0.85rem; margin-bottom: 1rem;">
      ...
      <div v-if="showDetailModal && invoiceDetails" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; justify-content: center; align-items: center; z-index: 1000; overflow-y: auto; padding: 2rem 0;">
        <div class="card" style="width: 100%; max-width: 500px; padding: 2.5rem; background: var(--card-bg); box-shadow: var(--shadow-lg); position: relative;">
          ...
          <div id="receipt-print-area" style="font-family: inherit; color: #000000; background: #ffffff; padding: 1.5rem; border: 1px solid #e2e8f0; border-radius: 8px;">
            <div style="text-align: center; border-bottom: 2px dashed #cbd5e1; padding-bottom: 1rem; margin-bottom: 1.5rem;">
              <h2 style="font-weight: 700; margin: 0; font-size: 1.5rem; color: #4f46e5;">HÓA ĐƠN TIỀN TRỌ</h2>
            ...
            <div style="border-top: 2px dashed #cbd5e1; padding-top: 1rem; font-size: 0.95rem;">
              <div style="display: flex; justify-content: space-between; font-weight: 700; font-size: 1.1rem; margin-bottom: 0.5rem;">
                <span>TỔNG CỘNG:</span>
                <span style="color: #4f46e5;">{{ formatMoney(invoiceDetails.totalAmount) }} VNĐ</span>
              </div>
              ...
              <div style="display: flex; justify-content: space-between; font-size: 0.85rem; font-weight: 600;">
                <span>Còn nợ:</span>
                <span style="color: #ef4444;">{{ formatMoney(invoiceDetails.totalAmount - invoiceDetails.paidAmount) }} VNĐ</span>
              </div>
            </div>
          </div>

<template>
  <div class="p-4 bg-bg-main min-h-full boarding-houses-page">
    ...
    <!-- Hiển thị nếu có lỗi xảy ra (Bổ sung thêm) -->
    <div v-else-if="error" class="flex flex-col items-center justify-center py-12 gap-4 text-rose-500">
      <AppIcon name="exclamation-circle" size="lg" />
      <span class="text-sm font-semibold">{{ error }}</span>
      <FormButton @click="fetchData" size="sm" variant="secondary">Thử lại</FormButton>
    </div>

    <template v-else>
      <div v-if="boardingHouses.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        <div v-for="house in boardingHouses" :key="house.id" @click="goToDetail(house.id)"
          ...
          <div class="px-5 py-3 flex-1 flex flex-col gap-2 text-xs">
            ...
            <div v-if="house.bankAccountNumber"
              class="pt-1 border-t border-border-main/40 flex items-center gap-1 text-[11px] text-text-sub">
              <span>🏦</span>
    ...
    <!-- Add/Edit Modal -->
    <Modal v-if="showAddModal || showEditModal" :title="showEditModal ? 'Cập Nhật Dãy Trọ' : 'Thêm Dãy Trọ Mới'"
      maxWidth="md" @close="closeModal">
      <form @submit.prevent="saveHouse">
        ...
        <div class="mb-4">
          <FormInput type="number" label="Ngày tính tiền cố định hàng tháng (1-31, để trống nếu tính theo ngày dọn vào)"
        ...
        <div class="mt-6 pt-4 border-t border-border-main">
          ...
          <div class="mb-4">
            <label class="block text-xs font-semibold text-text-main mb-1.5">Các quy tắc ra vào, vệ sinh,
              PCCC...</label>
            <textarea v-model="form.rules" rows="6" placeholder="Nhập nội quy của dãy trọ tại đây..."
              class="w-full px-3 py-2 text-xs border border-border-main bg-card text-text-main rounded-lg outline-none focus:border-primary focus:ring-1 focus:ring-primary/20"></textarea>
          </div>
        </div>

    ...
    <div v-else-if="room" class="flex flex-col gap-4">
      ...
      <div v-if="activeTab === 'info'" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-card border border-border-main rounded-2xl p-6 shadow-xs lg:col-span-2">
          ...
          <form @submit.prevent="handleSave" class="flex flex-col gap-4">
            <div>
              <label class="block text-xs font-semibold text-text-sub mb-1.5">Dãy trọ hiện tại (Không thể thay đổi)</label>
            ...
            <div>
              <label class="block text-xs font-semibold text-text-main mb-1.5">Số phòng / Tên phòng <span class="text-danger">*</span></label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-text-main mb-1.5">Giá thuê cơ bản (VNĐ) <span class="text-danger">*</span></label>
                <input type="number" v-model="form.basePrice" min="0" required
                  class="w-full px-3 py-2 text-xs border border-border-main bg-card text-text-main rounded-lg outline-none focus:border-primary focus:ring-1 focus:ring-primary/20" />
              </div>

              <div>
                <label class="block text-xs font-semibold text-text-main mb-1.5">Số người tối đa <span class="text-danger">*</span></label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-text-main mb-1.5">Chỉ số điện hiện tại (kWh) <span class="text-danger">*</span></label>
                <input type="number" v-model="form.currentElectricityIndex" min="0" required
                  class="w-full px-3 py-2 text-xs border border-border-main bg-card text-text-main rounded-lg outline-none focus:border-primary focus:ring-1 focus:ring-primary/20" />
              </div>

              <div>
                <label class="block text-xs font-semibold text-text-main mb-1.5">Chỉ số nước hiện tại (m³) <span class="text-danger">*</span></label>
        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="bg-card border border-border-main rounded-2xl p-6 shadow-xs">
            ...
            <div class="flex flex-col gap-4 text-xs">
              ...
              <div class="flex justify-between items-center py-1 border-b border-border-main/40">
                ...
                <span class="font-semibold text-text-main text-right truncate max-w-[150px]" :title="room.boardingHouse?.name">
                  {{ room.boardingHouse?.name }}
                </span>
              </div>
              <div class="flex justify-between items-center py-1">
                <span class="text-text-sub">Địa chỉ:</span>
                <span class="font-semibold text-text-sub text-right truncate max-w-[150px]" :title="room.boardingHouse?.address">
          ...
          <div class="bg-[rgba(0,102,204,0.04)] border border-primary/20 rounded-2xl p-5">
            ...
            <p class="text-[11px] leading-relaxed text-text-sub">
              Khi chỉnh sửa chỉ số điện/nước hiện tại của phòng, hệ thống sẽ sử dụng chỉ số này làm mốc tính chỉ số cũ cho lần lập hóa đơn tiền phòng tiếp theo.
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'history'" class="bg-card border border-border-main rounded-2xl p-4 shadow-xs">
        <div class="flex justify-between items-center mb-4 pb-2 border-b border-border-main/55">
          ...
          <span class="text-[11px] font-semibold text-text-sub px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-full">
        ...
        <DataTable
          ...
          :items="contracts"
          :loading="loading"
          loadingText="Đang tải lịch sử thuê phòng..."
          emptyText="Phòng trọ này chưa từng phát sinh hợp đồng thuê nào."
          clickable

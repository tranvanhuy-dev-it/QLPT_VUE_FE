        ...
        <div class="overflow-x-auto border border-border-main/50 rounded-xl bg-card">
          <table class="w-full text-sm text-left text-text-main border-collapse">
            <thead class="bg-slate-50 dark:bg-slate-900/40 border-b border-border-main/60">
              <tr>
                <th class="py-2 px-4 font-bold text-text-sub text-[11px] uppercase tracking-wider">Tên tài khoản
                  (username)</th>
            ...
            <tbody>
              <tr v-for="tenant in filteredTenants" :key="tenant.id"
                ...
                <td class="py-2.5 px-4">
                  <span
                    :class="['inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold', tenant.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/35 dark:text-emerald-400' : 'bg-rose-50 text-rose-600 dark:bg-rose-950/35 dark:text-rose-400']">
        ...
        <form @submit.prevent="createTenantAccount">
          <div class="flex flex-col gap-1.5 mb-4">
            ...
            <input type="text"
              ...
              v-model="form.username" placeholder="Ví dụ: nguyenvanan (viết liền không dấu)" required />
          </div>
          ...
          <div class="flex flex-col gap-1.5 mb-4">
            ...
            <input type="password"
              class="w-full px-3 py-2 border border-border-main rounded-lg bg-slate-50 dark:bg-slate-900 text-text-main text-sm outline-none focus:bg-white focus:border-primary"
              v-model="form.password" placeholder="Tối thiểu 6 ký tự" required />
          </div>
          ...
          <div class="flex flex-col gap-1.5 mb-4">
            ...
            <input type="text"
              class="w-full px-3 py-2 border border-border-main rounded-lg bg-slate-50 dark:bg-slate-900 text-text-main text-sm outline-none focus:bg-white focus:border-primary"
              v-model="form.fullName" placeholder="Nhập tên đầy đủ người ở" required />
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="flex flex-col gap-1.5">
              ...
              <input type="text"
                class="w-full px-3 py-2 border border-border-main rounded-lg bg-slate-50 dark:bg-slate-900 text-text-main text-sm outline-none focus:bg-white focus:border-primary"
                v-model="form.phone" placeholder="Số điện thoại liên hệ" />
            </div>
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <input type="email"
                class="w-full px-3 py-2 border border-border-main rounded-lg bg-slate-50 dark:bg-slate-900 text-text-main text-sm outline-none focus:bg-white focus:border-primary"
                v-model="form.email" placeholder="Địa chỉ email" />
            </div>
          </div>

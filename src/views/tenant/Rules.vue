    ...
    <div>
      ...
      <div v-else>
        ...
        <div v-if="!activeContract"
          ...
          ⚠️ Hiện tại tài khoản của bạn chưa được gắn vào hợp đồng thuê phòng nào. Vui lòng liên hệ chủ trọ để kích
          hoạt!
        </div>
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div class="lg:col-span-2 space-y-6">
            <div
              class="bg-card border border-border-main rounded-2xl shadow-xs overflow-hidden transition-all duration-200 hover:shadow-md">
              <!-- Header gradient bar -->
              ...
              <div class="p-6 md:p-8">
                ...
                <!-- Rules Content -->
                <div
                  class="text-sm md:text-base text-text-main/90 leading-relaxed whitespace-pre-line bg-slate-50/50 dark:bg-slate-900/50 rounded-xl p-5 md:p-6 border border-border-main/30 font-medium">
                  ...
                  <div v-else class="text-center py-12 text-text-sub flex flex-col items-center gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                      stroke="currentColor" class="w-12 h-12 text-text-sub/50">
                      <path stroke-linecap="round" stroke-linejoin="round"
                        d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z" />
                    </svg>
                    <span>Chủ trọ chưa cập nhật nội quy cụ thể cho dãy trọ này. Vui lòng tuân thủ các quy định văn hóa
                      chung và liên hệ chủ trọ khi cần giải đáp thắc mắc.</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="space-y-6">
            ...
            <div
              class="bg-card border border-border-main rounded-2xl p-6 shadow-xs hover:shadow-md transition-all duration-200">
              ...
              <div class="space-y-4 text-sm text-text-main">
                ...
                <div class="border-t border-border-main/30 pt-3">
                  ...
                  <div class="text-text-sub leading-normal">{{
                    activeContract.room.boardingHouse.address || 'Chưa cập nhật' }}</div>
                </div>
                <div class="border-t border-border-main/30 pt-3">
            ...
            <div
              class="bg-card border border-border-main rounded-2xl p-6 shadow-xs hover:shadow-md transition-all duration-200 bg-gradient-to-br from-card to-primary/5">
              ...
              <div class="space-y-3 text-sm text-text-main">
                <div class="flex items-center gap-3">
                  <div
                    class="w-8 h-8 rounded-full bg-success/10 text-success flex items-center justify-center font-bold text-sm shrink-0">
                    {{ activeContract.room.boardingHouse.landlord?.fullName?.charAt(0).toUpperCase() || 'C' }}
                  </div>
                  <div>
                    <div class="font-semibold text-text-main text-sm">{{
                      activeContract.room.boardingHouse.landlord?.fullName || 'Chủ trọ' }}</div>
                ...
                <div class="border-t border-border-main/30 pt-3 flex flex-col gap-2">
                  <a v-if="activeContract.room.boardingHouse.landlord?.phone"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                      stroke="currentColor" class="w-4 h-4 text-success">
                      <path stroke-linecap="round" stroke-linejoin="round"

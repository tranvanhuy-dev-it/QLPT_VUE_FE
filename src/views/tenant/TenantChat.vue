<template>
  <div class="h-full flex flex-col bg-bg-main p-4 lg:p-6 overflow-hidden">
    ...
    <div class="flex-grow bg-card border border-border-main rounded-2xl shadow-xs overflow-hidden flex flex-col min-h-0 max-w-4xl mx-auto w-full">
      ...
      <template v-else-if="!currentRoom">
        ...
        <div class="flex-grow flex flex-col items-center justify-center p-8 text-center text-text-sub select-none">
          <div class="w-16 h-16 rounded-full bg-slate-50 dark:bg-slate-950 flex items-center justify-center mb-4 border border-border-main">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.2" stroke="currentColor" class="w-8 h-8 text-primary/85 animate-pulse">
          ...
          <p class="text-xs max-w-[320px] leading-relaxed mb-4">
            Tài khoản của bạn chưa được liên kết với chủ nhà nào. Vui lòng liên hệ với chủ dãy trọ của bạn để được thêm vào hệ thống!
          </p>
          <router-link
            to="/tenant"
      ...
      <template v-else>
        <!-- Chat Header -->
        <div class="px-6 py-4 border-b border-border-main bg-slate-50/50 dark:bg-slate-900/10 flex items-center justify-between shrink-0">
          <div class="flex items-center gap-3.5">
            ...
            <!-- Landlord Avatar -->
            <div class="w-11 h-11 rounded-full bg-primary/10 text-primary border border-primary/20 font-bold flex items-center justify-center text-lg select-none">
        ...
        <div class="p-4 border-t border-border-main bg-card shrink-0">
          <form @submit.prevent="handleSendMessage" class="flex items-center gap-2">
            <input 
              ...
              type="text" 
              placeholder="Nhập nội dung tin nhắn gửi cho chủ nhà..." 

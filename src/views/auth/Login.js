  ...
  setup() {
    ...
    const renderFallbackGoogleButton = () => {
      ...
      if (btnContainer) {
        btnContainer.innerHTML = `
          <button type="button" class="btn btn-outline" style="width: 100%; display: flex; align-items: center; justify-content: center; gap: 0.75rem; border: 1px solid var(--border-color); border-radius: 2px; padding: 0.6rem 1rem; background: var(--card-bg); font-family: inherit; font-size: 0.9rem; font-weight: 500; cursor: pointer; color: var(--text-primary); transition: all 0.15s ease;">
    ...
    const renderCustomGoogleButton = () => {
      ...
      if (btnContainer) {
        ...
        btnContainer.querySelector('button').addEventListener('click', async () => {
          if (Capacitor.isNativePlatform()) {
            error.value = '';
            loading.value = true;
            try {

  ...
  <div class="auth-wrapper">
    <div class="auth-card">
      ...
      <form @submit.prevent="handleLogin">
        ...
        <div class="form-group" style="margin-bottom: 2rem;">
          ...
          <div style="position: relative; display: flex; align-items: center; width: 100%;">
            <input :type="showPassword ? 'text' : 'password'" class="form-input" v-model="password" placeholder="Nhập mật khẩu" required style="padding-right: 2.75rem;" />
            <button type="button" @click="showPassword = !showPassword" style="position: absolute; right: 0.875rem; background: none; border: none; cursor: pointer; color: var(--text-secondary); display: flex; align-items: center; justify-content: center; padding: 0;">
      ...
      <div style="margin-top: 2rem; display: flex; flex-direction: column; gap: 0.5rem; width: 100%;">
        <button type="button" @click="showSystemNotice = !showSystemNotice" class="btn btn-outline" style="width: 100%; font-size: 0.75rem; padding: 0.5rem; border: 1px dashed rgba(79, 70, 229, 0.3); background: rgba(79, 70, 229, 0.02); color: var(--primary-color); gap: 0.25rem; border-radius: 8px;">
          <span>⚙️ {{ showSystemNotice ? 'Ẩn thông báo hệ thống' : 'Xem thông báo hệ thống' }}</span>
        </button>
        
        <div v-if="showSystemNotice" style="padding: 0.75rem 1rem; border-radius: 12px; background-color: rgba(79, 70, 229, 0.05); border: 1px dashed rgba(79, 70, 229, 0.2); font-size: 0.75rem; color: var(--text-secondary); text-align: justify; line-height: 1.5; display: flex; flex-direction: column; gap: 0.25rem;">

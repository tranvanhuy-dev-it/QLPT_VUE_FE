  ...
  <aside class="app-sidebar">
    ...
    <div class="sidebar-header">
      ...
      <div class="brand-info">
        <h3 class="brand-name">Nhà Trọ Thông Minh</h3>
    ...
    <nav class="sidebar-nav">
      ...
      <template v-if="role === 'LANDLORD'">
        ...
        <router-link v-for="item in filteredLandlordGeneral" :key="item.to" :to="item.to" class="nav-item">
          <span class="nav-icon" v-html="item.icon"></span>
          <span class="nav-text">{{ item.label }}</span>
        </router-link>

        <div class="menu-group-label" v-if="filteredLandlordManage.length > 0">QUẢN LÝ</div>
        <router-link v-for="item in filteredLandlordManage" :key="item.to" :to="item.to" class="nav-item">
          <span class="nav-icon" v-html="item.icon"></span>
          <span class="nav-text">{{ item.label }}</span>
        </router-link>
      </template>
      ...
      <template v-if="role === 'TENANT'">
        ...
        <router-link v-for="item in filteredTenantGeneral" :key="item.to" :to="item.to" class="nav-item">
          <span class="nav-icon" v-html="item.icon"></span>
          <span class="nav-text">{{ item.label }}</span>
        </router-link>
      </template>

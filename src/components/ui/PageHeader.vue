  ...
  <div class="w-full flex flex-col gap-3 mb-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 w-full bg-card p-3 sm:p-3.5 rounded-2xl border border-border-main shadow-xs">
      ...
      <div class="flex items-center justify-between gap-3 sm:hidden w-full">
        ...
        <!-- Add Button (Mobile) -->
        <button v-if="showAdd" @click="$emit('add-click')" :disabled="disableAdd" class="inline-flex items-center px-3 py-1.5 text-[11px] font-extrabold rounded-lg bg-primary text-white hover:bg-primary-hover disabled:bg-slate-300 disabled:cursor-not-allowed cursor-pointer gap-1 transition-all shadow-xs shrink-0">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
          </svg>
          <span>{{ addText || 'Thêm' }}</span>
        </button>
      </div>
      ...
      <div class="flex flex-row items-center justify-between gap-2.5 w-full flex-nowrap">
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 flex-nowrap shrink-0">
          ...
          <button v-if="showAdd" @click="$emit('add-click')" :disabled="disableAdd" class="hidden sm:inline-flex items-center px-3 py-1.5 sm:px-3.5 sm:py-2 text-[11px] sm:text-xs font-extrabold rounded-lg bg-primary text-white hover:bg-primary-hover disabled:bg-slate-300 disabled:cursor-not-allowed cursor-pointer gap-1 sm:gap-1.5 transition-all shadow-xs shrink-0">

...
export default {
  ...
  setup(props) {
    ...
    const printReceipt = () => {
      const printContent = document.getElementById("receipt-print-area-shared").innerHTML;
      const printWindow = window.open("", "_blank");
      printWindow.document.write(`
        <html>
          <head>
            <title>Phiếu thanh toán tiền phòng & dịch vụ - Phòng ${props.invoice.contract?.room?.roomNumber}</title>
            <style>
              body {
    ...
    const downloadReceiptImage = async () => {
      const originalElement = document.getElementById("receipt-print-area-shared");
      ...
      headerDiv.innerHTML = `
        ...
        <div style="flex: 2; text-align: center;">Phiếu thanh toán tiền phòng & dịch vụ - Phòng ${props.invoice.contract?.room?.roomNumber}</div>
      ...
      try {
        ...
        const startDayStr = formatDate(props.invoice.billingPeriodStart).replace(/\//g, "-");
        const fileName = `PhieuThanhToan_Phong_${props.invoice.contract?.room?.roomNumber}_Ky_${startDayStr}.png`;
        ...
        try {
          ...
          if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
              ...
              text: `Hóa đơn tiền phòng phòng ${props.invoice.contract?.room?.roomNumber}`,
            });
            return;
          }
        } catch (shareErr) {
